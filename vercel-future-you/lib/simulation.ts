...
import { simulateMarketReturns } from './calculations';
...
const SCENARIO_TEMPLATES: MarketScenario[] = [
...
// Generate market events based on simulation duration
export function generateMarketEvents(startYear: number, endYear: number): SimulationEvent[] {
  const duration = endYear - startYear;
  const events: SimulationEvent[] = [];
  
  for (const template of SCENARIO_TEMPLATES) {
    if (template.year <= duration) {
      const impact = generateEventImpact(template.type);
      events.push({
...
// Process one quarter of simulation
export function processQuarter(
  ...
  // Find if there's an event this quarter
  ...
  const newAllocations: Allocation[] = portfolio.allocations.map(allocation => {
    ...
    const newValue = allocation.value * (1 + quarterlyReturn);
    return {
      ...allocation,
      value: newValue,
    };
  ...
  const totalBeforeContribution = newAllocations.reduce((sum, a) => sum + a.value, 0);
  
  // Distribute contribution according to allocation percentages
  ...
  // Update percentages based on new values
  newAllocations.forEach(allocation => {
...
export function applyDecision(
  ...
  params?: { assetClass?: AssetClass; amount?: number }
): SimulationState {
  ...
  // Find if we're in a crash or recovery
  const recentEvent = state.events.find(
    e => e.year === currentYear || e.year === currentYear - 1
  );
  ...
  switch (decisionType) {
    ...
    case 'sell':
      // Sell 20% of risky assets, move to cash
      ...
      if (cashIndex >= 0) {
        newAllocations[cashIndex].value += soldAmount;
      }
      description = 'Sold 20% of equity positions';
      wasGood = isInCrash ? false : null; // Selling in a crash is usually bad
      break;
      
    case 'buy-dip':
      // Move 30% of cash/bonds to equities
      ...
      if (equityIndex >= 0) {
        newAllocations[equityIndex].value += movedAmount;
      }
    ...
    case 'hold':
      ...
      wasGood = isInCrash ? true : null; // Holding during crash is good
      break;
  }
...
// Mock historical data adapter (for future CSV integration)
...
export function parseHistoricalCSV(csvContent: string): HistoricalDataPoint[] {
  // This would parse real CSV data
...
export function generateSyntheticHistory(
  ...
  for (let i = 0; i < years * 12; i++) {
    ...
    // Generate realistic-ish returns based on asset class
    const monthlyReturn = simulateMarketReturns(assetClass, 'normal') / 12;
