...
// Helper function to get scenarios suitable for a given time horizon
...
// Helper function to get a random scenario for a given horizon
...
// Helper to compress events for shorter time horizons
export function getCompressedEvents(scenario: MarketScenario, targetYears: number): ScenarioEvent[] {
  ...
  return scenario.events
    ...
    .map(e => ({
      ...e,
      yearOffset: Math.floor(e.yearOffset * ratio),
    }))
    .filter((e, i, arr) => 
      // Remove duplicate year events
      i === arr.findIndex(x => x.yearOffset === e.yearOffset && x.quarterOffset === e.quarterOffset)
    );
}

// Opponent AI behavior profiles for Arena mode
export type OpponentPersonality = 'cautious' | 'balanced' | 'greedy' | 'opportunistic';

export interface OpponentProfile {
  id: OpponentPersonality;
  name: string;
  description: string;
  decisionWeights: Record<string, number>;
}

export const OPPONENT_PROFILES: Record<OpponentPersonality, OpponentProfile> = {
  cautious: {
    id: 'cautious',
    name: 'The Conservative',
    description: 'Prefers safety over gains, rarely takes risks',
    decisionWeights: {
      'hold': 0.4,
      'rebalance': 0.25,
      'decrease-risk': 0.2,
      'increase-contribution': 0.1,
      'buy-dip': 0.03,
      'sell': 0.02,
    },
  },
  balanced: {
    id: 'balanced',
    name: 'The Strategist',
    description: 'Makes calculated decisions based on market conditions',
    decisionWeights: {
      'hold': 0.3,
      'rebalance': 0.25,
      'buy-dip': 0.15,
      'increase-contribution': 0.15,
      'decrease-risk': 0.1,
      'sell': 0.05,
    },
  },
  greedy: {
    id: 'greedy',
    name: 'The Risk Taker',
    description: 'Always chases maximum returns',
    decisionWeights: {
      'buy-dip': 0.35,
      'increase-equities': 0.25,
      'hold': 0.2,
      'increase-contribution': 0.15,
      'rebalance': 0.03,
      'sell': 0.02,
  ...
  opportunistic: {
    ...
    description: 'Adapts strategy based on recent market moves',
    decisionWeights: {
      'buy-dip': 0.25,
      'hold': 0.25,
      'rebalance': 0.2,
...
// Generate opponent decision based on personality and market conditions
export function generateOpponentDecision(
  personality: OpponentPersonality,
  eventType: string,
  currentReturn: number
): string {
  const profile = OPPONENT_PROFILES[personality];
  const weights = { ...profile.decisionWeights };
  
  // Adjust weights based on market conditions
  if (eventType === 'crash') {
    if (personality === 'cautious') {
      weights['sell'] = (weights['sell'] || 0) + 0.3;
      weights['decrease-risk'] = (weights['decrease-risk'] || 0) + 0.2;
    } else if (personality === 'greedy') {
      weights['buy-dip'] = (weights['buy-dip'] || 0) + 0.3;
    } else if (personality === 'opportunistic') {
      weights['buy-dip'] = (weights['buy-dip'] || 0) + 0.25;
    }
  } else if (eventType === 'bull') {
    if (personality === 'greedy') {
      weights['increase-equities'] = (weights['increase-equities'] || 0) + 0.2;
    } else if (personality === 'cautious') {
      weights['rebalance'] = (weights['rebalance'] || 0) + 0.2;
    }
  }
  
  // Adjust based on current performance
  if (currentReturn < -0.15) {
    weights['sell'] = (weights['sell'] || 0) + 0.1;
  } else if (currentReturn > 0.2) {
    weights['rebalance'] = (weights['rebalance'] || 0) + 0.1;
  }
  
  // Normalize weights
  const total = Object.values(weights).reduce((sum, w) => sum + w, 0);
  const normalizedWeights = Object.entries(weights).map(([action, weight]) => ({
    action,
    weight: weight / total,
  }));
  ...
  let cumulative = 0;
  for (const { action, weight } of normalizedWeights) {
    cumulative += weight;
