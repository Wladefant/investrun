...
export function AcademyAppInner({
  initialScreen = "enrollment",
  initialName = "Academy Student",
}: {
  ...
  const renderMission = () => {
    switch (activeMissionId) {
      case 1: return <Mission1Screen onComplete={(score, riskProfile) => handleMissionComplete(1, score, { riskProfile })} onBack={() => setScreen("dashboard")} />;
      case 2: return <Mission2Screen onComplete={(score, data) => handleMissionComplete(2, score, data)} onBack={() => setScreen("dashboard")} />;
      case 3: return <Mission3Screen onComplete={(score, crashBehavior) => handleMissionComplete(3, score, { crashBehavior })} onBack={() => setScreen("dashboard")} />;
      case 4: return <Mission4Screen onComplete={(score, data) => handleMissionComplete(4, score, data)} onBack={() => setScreen("dashboard")} />;
      case 5: return <Mission5Screen onComplete={(score, data) => handleMissionComplete(5, score, data)} onBack={() => setScreen("dashboard")} />;
      case 6: return <Mission6Screen onComplete={(score, data) => handleMissionComplete(6, score, data)} onBack={() => setScreen("dashboard")} />;
      default: return <MissionPlaceholderScreen missionId={activeMissionId} onComplete={(score) => handleMissionComplete(activeMissionId, score)} onBack={() => setScreen("dashboard")} />;
    }
  };
  ...
  return (
    <MobileLayout>
      ...
      {screen === "dashboard" && (
        <div className="flex-1 flex flex-col overflow-hidden">
          <AcademyDashboard progress={progress} onStartMission={handleStartMission} onProfileClick={() => setScreen("profile")} onTimeMachine={() => setScreen("time_machine")} />
        </div>
      )}

      {screen === "future_engine" && (
        <div className="flex-1 flex flex-col overflow-hidden">
          <FutureEngineScreen age={onboardingData?.age} initialContribution={onboardingData?.monthlyContribution} initialGoal={onboardingData?.selectedGoal} riskProfileId={onboardingData?.riskProfileId} />
        </div>
      )}

      {screen === "missions" && (
        <div className="flex-1 flex flex-col overflow-hidden">
          <MissionsScreen progress={progress} onStartMission={handleStartMission} onBack={() => setScreen("dashboard")} />
        </div>
      )}

      {screen === "mission" && (
        <div className="flex-1 flex flex-col overflow-hidden">{renderMission()}</div>
      )}

      {screen === "mission_result" && lastMissionResult && (
        <div className="flex-1 flex flex-col overflow-hidden">
          <MissionResultScreen {...lastMissionResult} rankInfo={getCurrentRank(progress.xp)} onContinue={() => setScreen("dashboard")} onViewDNA={progress.completedMissions.length >= 2 ? () => setScreen("dna_report") : undefined} />
        </div>
      )}

      {screen === "dna_report" && (
        <div className="flex-1 flex flex-col overflow-hidden">
          <DNAReportScreen progress={progress} onBack={() => setScreen("dashboard")} />
        </div>
      )}
      ...
      {screen === "solo" && (
        <div className="flex-1 flex flex-col overflow-hidden"><SoloScreen /></div>
      )}
      ...
      {screen === "profile" && (
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <h2 className="text-lg font-bold text-foreground mb-1">{progress.playerName}</h2>
            <p className="text-sm text-muted-foreground mb-4">{getCurrentRank(progress.xp).icon} {getCurrentRank(progress.xp).label} · {progress.xp} XP</p>
            <p className="text-xs text-muted-foreground">{progress.completedMissions.length} of 7 missions completed</p>
          </div>
        </div>
      )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <ArenaScreen
            ...
            onStatsUpdate={(stats, xpEarned) => {
              setProgress(p => ({ ...p, arenaStats: stats, xp: p.xp + xpEarned, currentRank: getCurrentRank(p.xp + xpEarned).id }));
            }}
          />
        </div>
      )}
      ...
      {showNav && (
        <BottomNav activeTab={activeTab} onNavigate={(tab) => setScreen(tab as Screen)} onProfessorClick={() => setShowProfessor(true)} />
      )}
      ...
      <ProfessorChat
        ...
        onNavigate={(s) => { setShowProfessor(false); setScreen(s as Screen); }}
      />
    </MobileLayout>
  );
}
